...
int main(int argc, char* argv[]) {
    ...
    if (width <= 0 || height <= 0 || maxIterations <= 0 || radius <= 0 || sigma <= 0.0) {
        std::cerr << "Uso invalido.\n";
        std::cerr << "Ejemplo: ./mandelbrot_secuencial 7680 4320 1000 15 6.0\n";
        return 1;
    }
    ...
    savePPM("mandelbrot_original.ppm", image, width, height);
    ...
    savePPM("mandelbrot_filtrado.ppm", filteredImage, width, height);
    ...
    std::cout << "Resultados de tiempo secuencial:\n";
    std::cout << "Tiempo Tarea A - Mandelbrot: " << mandelbrotTime.count() << " segundos\n";
    std::cout << "Tiempo Tarea B - Convolucion 2D: " << convolutionTime.count() << " segundos\n";
    std::cout << "Tiempo total: " << totalTime << " segundos\n";

    std::cout << "\nArchivos generados:\n";
    std::cout << "mandelbrot_original.ppm\n";
    std::cout << "mandelbrot_filtrado.ppm\n";
